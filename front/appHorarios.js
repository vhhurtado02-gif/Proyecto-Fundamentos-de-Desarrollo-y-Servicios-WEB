...
async function cargarHorarioParaEditar() {
  ...
  if (!id||id<=0) { mostrarToast("Ingresa un ID válido.", "error"); return; }
  try {
    const r = await apiFetch("GET","",null);
    if (!r.datos.ok) { mostrarToast("Error al buscar.", "error"); return; }
    const h = (r.datos.datos||[]).find(function(x){ return x.idHorario == id; });
    if (!h) { mostrarToast("No se encontró un horario con ese ID.", "error"); return; }
...
async function ejecutarBorrarHorario() {
  ...
  if (!id||id<=0) { mostrarToast("Ingresa un ID válido.", "error"); return; }
  const confirmado = await mostrarConfirmacion("Eliminar horario", "¿Seguro que deseas eliminar el horario con ID " + id + "? Esta acción no se puede deshacer.");
...
function aplicarEstilos() {
  ...
  s.textContent = `
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .cabecera-contenido { display: flex; align-items: center; gap: 16px; max-width: 860px; margin: 0 auto; }
    .cabecera-logo { font-size: 2.2em; }
    #tituloPrincipal { margin: 0; font-size: 1.4em; color: #4a9eff; font-weight: 700; letter-spacing: 0.5px; }
    #subtituloApp { margin: 4px 0 0; font-size: 0.9em; color: #8ab4d4; }

    /* ── Área Principal ───────────────────── */
    #areaPrincipal { padding: 24px 16px; max-width: 860px; margin: 0 auto; }
    h2 {
    ...
    /* ── Tabla ────────────────────────────── */
    .tabla-contenedor { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; border-radius: 10px; box-shadow: 0 4px 16px rgba(0,0,0,0.3); }
    ...
    @media(max-width:400px) {
      #tituloPrincipal { font-size: 1.1em; }
      #tablaHorarios { font-size: 0.75em; }
      #tablaHorarios td, #tablaHorarios th { padding: 6px 8px; }
      .btn-menu { padding: 12px 14px; }
    }
  `;
